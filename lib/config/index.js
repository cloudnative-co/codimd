...
config.isBoxEnable = config.box.boxAppSettings.clientID && config.box.boxAppSettings.clientSecret && config.box.boxAppSettings.appAuth
...
if (['filesystem', 's3', 'minio', 'imgur', 'azure', 'lutim', 'box'].indexOf(config.imageUploadType) === -1) {
  logger.error('"imageuploadtype" is not correctly set. Please use "filesystem", "s3", "minio", "azure", "lutim", "box" or "imgur". Defaulting to "filesystem"')
...
config.allowedUnfoldCodeMimeType = [
    'text/css',
    'text/html',
    'text/x-c',
    'text/x-c++hdr',
...
if (config.isBoxEnable && config.contentBoxUpload) {
  config.allowedBoxUploadMimeTypes = [
...
console.log(config.allowedBoxUploadMimeTypes)
