  ...
  var Note = sequelize.define('Note', {
    ...
    hooks: {
      ...
      afterUpdate: function (note, options, callback) {
        try {
            const parsed = metaMarked(note.content)
            ...
            sequelize.models.Metadata.destroy({
                where: {noteId: noteId}
            }).then((metadatas) =>{
                return sequelize.models.Metadata.bulkCreate(records).catch((err) => {
                    logger.info(err)
                    throw err;
                })
            }).catch((err) => {
